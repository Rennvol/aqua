...
func migrateFromJSON() {
	...
	// kv fields
	for _, k := range []string{"pin", "oled_line1", "oled_line2", "oled_line3", "oled_line4", "oled_line5", "oled_line6", "oled_line7", "oled_line8", "oled_text_custom", "poll_interval", "cron_api_key", "cron_token", "public_url", "clock_on", "clock_always_on", "clock_cron_job_id", "oled_on"} {
		if v, ok := raw[k]; ok {
			var s string
...
func syncSettingsFromDB() {
	...
	if v := kvGet("oled_line5", ""); v != "" {
		settings.OLEDLine5 = v
	}
	if v := kvGet("oled_line6", ""); v != "" {
		settings.OLEDLine6 = v
	}
	if v := kvGet("oled_line7", ""); v != "" {
		settings.OLEDLine7 = v
	}
	if v := kvGet("oled_line8", ""); v != "" {
		settings.OLEDLine8 = v
	}
	if v := kvGet("oled_text_custom", ""); v != "" {
